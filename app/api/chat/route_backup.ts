...
function shouldForceClarification(question: string): string | null {
  ...
  const mentionsBaltazar = /\bbaltazar\b|\bbaltazaru\b/.test(q);
  ...
  if (
    asksPrice &&
    (mentionsStudy || mentionsBaltazar || /^(cijena|skolarina|školarina)$/.test(q)) &&
  ...
  if (
    asksDuration &&
    (mentionsStudy || mentionsBaltazar || /^(trajanje|koliko traje)$/.test(q)) &&
    !findSpecificProgramMention(q)
  ) {
  ...
  if (
    asksAdmissions &&
    (mentionsStudy || mentionsBaltazar || q === 'uvjeti za upis' || q === 'uvjeti') &&
    !findSpecificProgramMention(q)
  ) {
  ...
  if (
    asksDates &&
    (mentionsStudy || mentionsBaltazar || q === 'termini upisa' || q === 'termini') &&
    !findSpecificProgramMention(q)
  ) {
...
function buildContextualRetrievalQuery(messages: ChatMessage[], latestQuestion: string): string {
  ...
  if (!followUp || !previousUser?.content) return latestQuestion;
...
async function resolveQuestion(messages: ChatMessage[], latestQuestion: string): Promise<ResolvedQuery> {
  ...
  const contentGroup = resolveContentGroup(messages, latestQuestion);
  const requestedSectionType = detectRequestedSectionType(latestQuestion);
...
function getUrlPriority(
  ...
  if (contentGroup === 'upisi') {
    if (requestedSectionType === 'cijena') {
      ...
      if (url.includes('/upisi/o-upisima-i-studiranju-na-baltazaru/')) return 2;
    ...
    if (requestedSectionType === 'uvjeti' || requestedSectionType === 'upis') {
      if (url.includes('/upisi/procedura-upisa/')) return 6;
      if (url.includes('/upisi/o-upisima-i-studiranju-na-baltazaru/')) return 4;
      if (url.includes('/upisi/postupak-i-termini-upisa/')) return 3;
      if (url.includes('/upisi/cesta-pitanja/')) return 1;
    }
    ...
    if (url.includes('/upisi/o-upisima-i-studiranju-na-baltazaru/')) return 3;
    if (url.includes('/upisi/cesta-pitanja/')) return 1;
  }
...
function formatBulletListFromChunk(chunk: RetrievedChunk, intro: string): string | null {
  ...
  const formatted = unique.map((line, i) => `${i + 1}. ${line}`).join('\n');
  return `${intro}\n${formatted}\n\nIzvor: ${chunk.url}`;
}
...
function extractFactFirstAnswer(
  ...
  if (intent === 'nositelj') {
    ...
    for (const chunk of sectionPool) {
      const maybe = formatBulletListFromChunk(chunk, 'Nositelj ili izvođač koji je naveden u izvoru je:');
      if (maybe) return maybe;
    }
    return STRICT_FALLBACK;
  }
  ...
  if (intent === 'popis_cjelozivotnih') {
    const relevant = chunks.filter(
      (c) => c.content_group === 'cjelozivotno_obrazovanje' && c.entity_name
    );

    const names = Array.from(new Set(
      relevant.map((c) => c.entity_name?.trim()).filter(Boolean) as string[]
    ));

    if (names.length) {
      const sourceUrl =
        relevant.find((c) => c.url?.includes('/cjelozivotno-obrazovanje/'))?.url ??
        relevant[0]?.url ??
        null;

      return `Programi cjeloživotnog obrazovanja koji su navedeni u izvorima su:\n${names.map((n, i) => `${i + 1}. ${n}`).join('\n')}\n\nIzvor: ${sourceUrl}`;
    }
  }
  ...
  if (intent === 'popis_studijskih') {
    const relevant = chunks.filter(
      (c) =>
        (c.content_group === 'studijski_programi' ||
          c.content_group === 'prijediplomski_studiji' ||
          c.content_group === 'diplomski_studiji' ||
          c.content_group === 'specijalisticki_studiji') &&
        c.entity_name
    );
    ...
    if (names.length) {
      const sourceUrl = relevant[0]?.url ?? null;
      return `Studijski programi koji su navedeni u izvorima su:\n${names.map((n, i) => `${i + 1}. ${n}`).join('\n')}\n\nIzvor: ${sourceUrl}`;
    }
  }
...
function buildSystemPrompt(
  ...
  const answerInstruction = [
    factSeeking
      ? 'Ovo je pitanje koje traži konkretnu informaciju. Odgovori samo onim što je jasno navedeno u izvorima.'
      : 'Odgovaraj isključivo na temelju izvora u nastavku.',
    'Ne smiješ dodavati činjenice koje nisu jasno sadržane u izvorima.',
    'Ako točan odgovor nije naveden, nemoj nagađati.',
    'Ako izvor sadrži samo povezanu informaciju, smiješ reći da detalj nije naveden, ali da postoje povezane informacije iz istog izvora.',
    resolved.preferredUrl
      ? `Prvenstveno se osloni na ovaj isti izvor prethodne teme: ${resolved.preferredUrl}`
      : 'Zadrži istu temu razgovora kad je riječ o follow-up pitanju.',
    resolved.resolvedEntityName
      ? `Aktivni entitet je: ${resolved.resolvedEntityName}`
      : 'Ako je moguće, zadrži isti entitet.',
    resolved.requestedSectionType
      ? `Tražena sekcija je: ${resolved.requestedSectionType}`
      : 'Nije eksplicitno zadana sekcija.',
    `Ako nema dovoljno podataka, odgovori točno: "${STRICT_FALLBACK}"`,
  ].join('\n');

  const conversationInstruction = [
    resolved.isFollowUp
      ? 'Ovo izgleda kao nastavak prethodnog razgovora. Zadrži temu i isti entitet.'
      : 'Odgovori prema trenutno postavljenom pitanju.',
    `Aktivna domena razgovora: ${resolved.contentGroup}.`,
    resolved.preferredUrl
      ? `Aktivni prethodni izvor: ${resolved.preferredUrl}.`
      : 'Nema prethodnog preferiranog izvora.',
    'Jasno razlikuj: studijske programe, online studij, cjeloživotno obrazovanje, kolegije, nastavnike i opće informacije o upisu.',
  ].join('\n');
  ...
  return [
    ...
    '',
    'Ako odgovaraš na temelju izvora, na kraju obavezno dodaj redak:',
    'Izvor: <točan URL izvora>',
    'Ako koristiš više izvora, navedi svaki URL u zasebnom retku.',
    'Odgovor mora biti kratak, jasan i akademski pristojan.',
    '',
    'IZVORI:',
    context,
    '',
...
export async function POST(req: NextRequest) {
  try {
    ...
    const forcedClarification = shouldForceClarification(userMessage.content);
    ...
    return new Response(fullText, { status: 200, headers: NO_CACHE_HEADERS });
  } catch (error) {
